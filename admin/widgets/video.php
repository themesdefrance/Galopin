<?php
class GalopinVideo extends WP_Widget{
	...
	public function __construct(){
		parent::__construct(
			...
			'GalopinVideo',
			__('Galopin - Video', 'galopin'),
			array('description'=>__('Add a video from Youtube, Dailymotion or Vimeo.', 'galopin'))
		);
	}
	...
	public function widget($args, $instance){
		...
		?>
				...
				<?php }else{ ?>
					...
					<p><?php _e('To display a video, please add a Youtube, Dailymotion or Vimeo URL in the widget settings.', 'galopin'); ?></p>
	...
	public function form($instance){
		...
		if ($this->novideo){
			echo '<div class="error">';
			_e('This video url doesn\'t seem to exist.', 'galopin');
			echo '</div>';
		}
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'video_link' ); ?>"><?php _e( 'Video URL (Youtube, Dailymotion or Vimeo):', 'galopin' ); ?></label> 
...
if (!function_exists('galopin_video_widget_init')){

	function galopin_video_widget_init(){
	
		register_widget('GalopinVideo');
	}
}
add_action('widgets_init', 'galopin_video_widget_init');
