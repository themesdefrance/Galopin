...
if (!function_exists('galopin_single_post_nav')){
	function galopin_single_post_nav() {
		...
		<nav class="entry-navigation" role="navigation">
			<?php
			if ( is_attachment() ) :
				previous_post_link( '<span class="meta-nav-prev">%link</span>', __( 'Published In', 'galopin' ) . '%title' );
			else :
				previous_post_link( '<span class="meta-nav-prev">%link</span>', __( 'Previous Post', 'galopin' ));
				next_post_link( '<span class="meta-nav-next">%link</span>', __( 'Next Post', 'galopin' ));
			endif;
			?>
